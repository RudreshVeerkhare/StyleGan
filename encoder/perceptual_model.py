...
class PerceptualModel:
    ...
    def build_perceptual_model(self, generated_image_tensor):
        ...
        self.features_weight_placeholder = tf.placeholder(
            self.features_weight.dtype, shape=self.features_weight.get_shape())
        self.img_features_placeholder = tf.placeholder(
            self.ref_img_features.dtype, shape=self.ref_img_features.get_shape())

        self.features_weight_op = self.features_weight.assign(
            self.features_weight_placeholder)
        self.img_features_op = self.ref_img_features.assign(
            self.img_features_placeholder)
    ...
    def set_reference_images(self, images_list):
        ...
        if len(images_list) != self.batch_size:
            ...
            empty_features_shape = [self.batch_size -
        ...
        self.sess.run(self.features_weight_op, {
                      self.features_weight_placeholder: weight_mask})
        self.sess.run(self.img_features_op, {
                      self.img_features_placeholder: image_features})
